...
def ab_clean(text): #cleans up the abstract given in xml cause it's full of symbols and extraneous text
    ...
    text = text[:end]
    to_replace = ['xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:p1="http://pubmed.gov/pub-one">',
        '<p>',
        '<p',
        '</p>',
        '<b>',
...
def rss_scrape(rss_urls): #from a given rss url returns a list of lists with elements [pmid, title, abstract]
    ...
    for url in rss_urls:
        ...
        for item in soup.find_all('item'):
            ...
            if pmid in exclusion_set: #check for pmid in exclusion_set. Only continue to other data if pmid is NOT in exclusion_set
                pass
            else:
